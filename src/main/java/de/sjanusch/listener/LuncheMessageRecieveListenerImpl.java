...
/**
 * Created by Sandro Janusch Date: 18.05.16 Time: 20:33
 */
public class LuncheMessageRecieveListenerImpl implements LuncheMessageRecieveListener {
  ...
  @Inject
  public LuncheMessageRecieveListenerImpl(final SuperlunchRequestHandler superlunchRequestHandler, final TextHandler textHandler,
      final BotConfiguration botConfiguration, final MessageRecieverBase messageRecieverBase,
      final LunchMessageProtocol lunchMessageProtocol) {
  ...
  private String createMittagessenMessage(final Lunch lunch, final String actualUser) {
    ...
    stringBuilder.append("(" + lunch.getCreatorName() + ", " + this.convertVeggyValue(lunch.isVeggy()) + ", " + "<a href=\""
        + lunch.getDetailLink() + "\">Details</a>)");
    stringBuilder.append("</li>");
    return stringBuilder.toString();
  }
