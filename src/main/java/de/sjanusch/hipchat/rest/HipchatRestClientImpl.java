...
public class HipchatRestClientImpl implements HipchatRestClient {
    ...
    @Override
    public void hipchatRestApiSendPrivateMessage(final HipchatMessage chatMessage) {
        try {
            final String path = "/user/" + "sjanusch@seibert-media.net" + "/message";
            this.hipchatRestApPrivateMessage(buildClient().target(this.getHipchatRestApi()).path(path), chatMessage);
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        } catch (KeyManagementException e) {
            e.printStackTrace();
    ...
    @Override
    public void hipchatRestApiSendPrivateNotification(final HipchatMessage chatMessage) {
        try {
            final String path = "/user/" + "sjanusch@seibert-media.net" + "/message";
            this.hipchatRestApiPrivateNotification(buildClient().target(this.getHipchatRestApi()).path(path), chatMessage);
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        } catch (KeyManagementException e) {
            e.printStackTrace();
    ...
    private void hipchatRestApPrivateMessage(final WebTarget target, final HipchatMessage chatMessage) throws IOException {
        logger.debug("Requesting  '" + target.getUri() + "' by POST ");
        try {
            Response response = target.request(MediaType.APPLICATION_JSON_TYPE).header("Authorization", "Bearer " + hipchatConfiguration.getHipchatRestApiKeyMessage())
                .post(Entity.entity(chatMessage, MediaType.APPLICATION_JSON_TYPE));
    ...
    private void hipchatRestApiPrivateNotification(final WebTarget target, final HipchatMessage chatMessage) throws IOException {
        logger.debug("Requesting  '" + target.getUri() + "' by POST ");
        try {
            Response response = target.request(MediaType.APPLICATION_JSON_TYPE).header("Authorization", "Bearer " + hipchatConfiguration.getHipchatRestApiKeyMessage())
                .post(Entity.entity(chatMessage, MediaType.APPLICATION_JSON_TYPE));
