...
public class HipchatRoomInfo {
  ...
  private static final Logger logger = LoggerFactory.getLogger(HipchatRoomInfo.class);
  ...
  public static HipchatRoomInfo getInfo(final String APIKey, final Room room) {
    if (APIKey.equals(""))
      return null;
    final HipchatRoomInfo[] data = getRooms(APIKey);
    if (data == null)
      return null;
    for (final HipchatRoomInfo h : data) {
      if (h.equals(room))
        return h;
    }
    return null;
  ...
  public static HipchatRoomInfo[] getRooms(final String APIKey) {
    return getRoomHolder(APIKey).rooms;
  }

  private static RoomHolder getRoomHolder(final String APIKey) {
    try {
      ...
      final RoomHolder r = new RoomHolder();
  ...
  public boolean equals(final Object obj) {
    ...
    if (obj instanceof HipchatRoomInfo) {
      final HipchatRoomInfo r = (HipchatRoomInfo) obj;
      return r.name.equals(name) && r.is_private == is_private && r.created == created && r.xmpp_jid.equals(xmpp_jid)
          && r.last_active == last_active && r.is_archived == is_archived && r.owner_user_id == owner_user_id && r.room_id == room_id
